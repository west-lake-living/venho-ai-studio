...
class InstagramAdapter(BasePlatformAdapter):
    ...
    def build_payload(self, request: PublishingRequest) -> dict:
        return {
            "caption": request.content.text_prose,
            "children": request.content.media_urls,
    ...
    def publish(self, request: PublishingRequest, dry_run: bool = True) -> PlatformResult:
        ...
        return PlatformResult(platform=self.platform, success=False, status="FAILED", error_code=ERR_ADAPTER_FAILED, error_message="real Instagram API publishing requires controlled manual test", payload=payload)
