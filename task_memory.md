...
- **18/09 — Threads kẹt ở `GATEWAY_ACCEPTED`:** `pub-friday-threads-21ff805d` (slot 2026-09-18) dispatch xong nhưng không bao giờ lên `PUBLISHED`. Đọc thẳng `publishing_gateway/adapters/make_gateway.py::interpret_make_response()` — comment gốc của tác giả đã giải thích rõ: route Make.com nào chưa gắn module "Webhooks > Webhook response" ở cuối (khác FB/IG đã có) thì Make chỉ trả "Accepted" chung chung, không có `platform_post_id` — code cố ý coi đó là `GATEWAY_ACCEPTED` optimistic, KHÔNG phải lỗi ("a Threads route that has not been built yet must not start reporting failures"). Kiểm tra registry: có 4 bài Threads khác cũng kẹt y hệt từ giữa tháng 8 (15/08, 17/08, 19/08) — không phải sự cố mới, mà 1 khoảng trống tồn tại cả tháng, chỉ là hôm nay mới có bài Threads mới đủ điều kiện dispatch lại.
- **Hỏi Harry trước khi sửa gì:** xác nhận hiện chưa đăng Threads → đặt `cadence.threads: 0` trong `config/projects/venho_hotel/content/calendar_rules.yaml` (commit `95176ea`). Trong lúc làm phát hiện thêm: `config/projects/venho_hotel/publishing/platforms.yaml` đã ghi `threads.enabled: false` từ trước, nhưng **cờ này chết hoàn toàn** — đường dispatch thật (`growth_orchestrator/bridges/m07_publishing_bridge.py::M07PublishingBridge`) route facebook/instagram/threads qua CHUNG 1 `MakeGatewayAdapter`/1 webhook (`MAKE_GROWTH_WEBHOOK_URL`), không hề đọc `platforms.yaml`; file đó chỉ được `publishing_gateway/contract_validator.py::validate_contract()` đọc, và hàm đó chỉ được gọi từ `publishing_gateway/gateway_router.py` — một đường CLI khác (`publishing_gateway/cli.py`), không phải đường Growth Agent đang chạy sản xuất. Kết luận: đặt `threads: enabled: false` trước đây chưa từng có tác dụng chặn dispatch thật.
- **19/09 — Zalo kẹt ở `DISABLED`, cùng bản chất:** `pub-saturday-zalo-1d8be4a8` (slot 2026-09-19) → `ZaloOAAdapter(enabled=bool(zalo_url))` tự tắt vì `MAKE_ZALO_WEBHOOK_URL`/`ZALO_APP_ID`/`ZALO_APP_SECRET`/`ZALO_REFRESH_TOKEN` đều rỗng trong GitHub Secrets (đã thấy trống trong env dump của run log từ hôm trước). Harry xác nhận: "Zalo OA chưa sẵn sàng". Registry cũng có 3 bài Zalo cũ kẹt `DISABLED` y hệt (từ 28/08) — cùng loại tồn kho như Threads.
- **Kiểm tra không còn tồn đọng:** sau 2 lần dispatch (18/09 xử lý nốt Threads, 19/09 xử lý nốt Zalo), registry không còn publication nào ở `APPROVED_SCHEDULED`/`PENDING_APPROVAL` cho Threads hay Zalo — chỉ còn Facebook/Instagram. Workflow sinh nội dung thật (`growth-daily-cycle.yml` → `venho-growth weekly-cycle --platform facebook --platform instagram`) vốn dĩ **chưa từng truyền `--platform threads` hay `--platform zalo`** — nên khả năng cao 2 bài lỗi này là tàn dư cuối cùng của 1 đợt sinh nội dung hồi 28/08 (khi có thể Threads/Zalo còn nằm trong danh sách platform), không phải lỗi đang tiếp diễn.
- **Sửa gốc (Harry chọn "Sửa ngay" khi được hỏi, vì đây là dạng lỗi chắc chắn tái diễn với bất kỳ platform mới nào chưa cấu hình xong — không muốn vá thủ công từng lần):** `growth_orchestrator/cli.py::dispatch_due_cmd` cũ có `accepted_statuses = {"PUBLISHED"} if require_dispatch else {"GATEWAY_ACCEPTED", "PUBLISHED"}` rồi `failed = [p for p in publications if p.status not in accepted_statuses]` — nghĩa là `DISABLED` luôn bị tính là lỗi (kể cả khi KHÔNG có `--require-dispatch`, vì dòng `if not ok: raise typer.Exit(code=1)` chạy vô điều kiện, không phụ thuộc cờ), và `GATEWAY_ACCEPTED` bị tính là lỗi khi CÓ `--require-dispatch` — mâu thuẫn trực tiếp với chủ đích đã ghi trong `interpret_make_response`'s docstring rằng optimistic-accept không được coi là lỗi.
  - Tách logic thành hàm thuần `dispatch_due_outcome(publications, *, require_dispatch) -> (ok, failed)` đặt trước command trong `cli.py` — dễ unit-test độc lập, không cần dựng registry/bridge/tmp_path đầy đủ như test cũ trong `test_growth_approve_and_dispatch.py`.
  - Định nghĩa lại "lỗi thật" = chỉ `GATEWAY_ERROR_STATUS`/`NEEDS_REVISION_STATUS` (import từ `approve_and_dispatch.py`, không định nghĩa lại string literal). `DISABLED`/`GATEWAY_ACCEPTED`/`SHADOW_HELD`/`PUBLISHED` đều không còn bị tính là lỗi.
  - `require_dispatch=True` vẫn giữ đúng ý nghĩa gốc "phải có ít nhất 1 bài due" (`ok = bool(publications) and not failed`) — không đổi phần đó, chỉ đổi định nghĩa `failed`.
- **Test:** 3 test mới trong `tests/test_growth_approve_and_dispatch.py` — tái tạo chính xác batch ngày 18/09 (facebook PUBLISHED + threads GATEWAY_ACCEPTED + zalo DISABLED → phải `ok=True`), xác nhận `GATEWAY_ERROR` thật vẫn `ok=False`, và batch rỗng dưới `--require-dispatch` vẫn `ok=False`. Import trực tiếp `dispatch_due_outcome` từ `growth_orchestrator.cli`, không cần mock `M07PublishingBridge`/`PublicationRegistry`.
- **Môi trường chạy test:** server không có `.venv` sẵn cho repo này (thư mục tồn tại nhưng rỗng — có lẽ path convention từ máy Mac không áp dụng ở đây) và không có `pip`/`python3-venv` cài đặt hệ thống (không có quyền `sudo apt`). Dùng `uv venv` + `uv pip install -e . pytest` dựng venv tạm ở `/tmp` — `uv` đã có sẵn ở `~/.local/bin/uv`. `pytest tests/test_growth_approve_and_dispatch.py`: 54/54 pass. `pytest tests/test_growth_*.py tests/test_cli.py`: 369/372 pass — 3 fail còn lại (`test_run_daily_cycle_saturday_generates_real_image_with_injected_provider`, `test_run_daily_cycle_uploads_validated_image_to_drive_and_stores_public_url`, `test_expired_token_uses_secret_client_config_before_refresh`) đều do thiếu `googleapiclient` (chưa cài extra `[drive]` trong venv tạm) — xác nhận không liên quan bằng `git stash` chạy lại đúng 2 test đó trên code cũ, fail giống hệt.
- **Push khó hơn dự kiến — không phải do lỗi, do 2 workflow khác commit chen ngang liên tục:** `git push` thường (không set token) bị 403 ("Permission to west-lake-living/venho-ai-studio.git denied to harryhpham") — credential mặc định của server chỉ đọc. Dùng `git -c credential.helper= push https://x-access-token:$GH_TOKEN@github.com/...` (định dạng bearer header đơn giản KHÔNG hoạt động, phải dùng dạng URL-embedded `x-access-token:`). Bị `non-fast-forward` 2 lần liên tiếp vì `growth-publish-scheduler.yml`/`growth-research-cycle` tự commit song song đúng lúc đang làm (`chore: growth scheduled dispatch ...`, `chore: research cycle ...`) — mỗi lần phải `fetch` + `git merge-base --is-ancestor` (xác nhận an toàn) + `git rebase FETCH_HEAD` rồi push lại.
- **Không tự resolve 1 conflict không phải của mình:** trước khi rebase lần đầu, repo có sẵn 3 file thay đổi cục bộ chưa commit (`publication_registry.json`, `research/proposed_facts.json`, `research/trend_candidates.json`) — không phải do phiên này tạo ra (có thể từ 1 lần chạy CLI/cron cục bộ trước đó trên server). Đã `git stash push -u` trước khi rebase để không mất; khi `git stash pop` lại bị conflict với bản mới hơn vừa kéo về từ remote (`publication_registry.json`) — thay vì tự đoán cách merge đúng cho dữ liệu registry sản xuất, dùng `git reset --merge` để huỷ pop, giữ nguyên stash entry (`stash@{0}`) an toàn, để Harry/người tạo ra thay đổi đó tự xem lại bằng `git stash show -p stash@{0}`.
...
- Resumed at T2 after the human-reported HARRY-ROG recovery. Minimal approved
  health check passed: Tailscale/ComfyUI/GPU/workflow all PASS; 5132 MiB VRAM
  free. T0 was not rerun and no restart was attempted.
- Exactly one B05 artifact was materialized under the unchanged P12 contract:
  denoise `0.35`, CFG `6.0`, steps `21`, A2, Candidate v3 workflow hash
  `53dc090691b8feac2a8b8a4309d43af737e304b09330e072b4ab5632ed5aad91`.
  Artifact hash: `ce58f0ac97a74bc07eccfba9d8c96584ff38eafd5a0a53e14fb84d363a873e40`.
- T3 lineage/regression PASS: artifact, manifest, contract, source,
  workflow, A2, parameters, and protected quality state all matched. Offline
  validation recorded `15 passed`, compileall PASS, and diff check PASS.
- Exactly one Gemini FACE_LOCAL call was attempted and returned invalid before
  provider response because `GEMINI_API_KEY`/`GOOGLE_API_KEY` was absent.
  `PROVIDER_HOLD=ACTIVE`; provider calls `1`, retries `0`. No second call,
  retry, alternate provider, Nano call, or follow-up was created.
