...
|---|---:|---|
| `venho-ai-studio` | `PYTHONPATH=. /usr/bin/python3 -m pytest -q` → **454/454 pass**, 0 API call | current commit |
...
## 2.1 Tests và runtime (snapshot giữa quá trình triển khai — đã superseded bởi §0)

> Bảng này là snapshot chụp giữa lúc triển khai roadmap v1.5, **trước** khi Phase 0–6 và closeout QA-01/DOC-01 hoàn tất trong cùng ngày. Con số hiện hành, chính thức là bảng ở §0 (454/454 và 65/65, khớp `task_status.md`). Giữ bảng dưới đây làm lịch sử tiến trình, không dùng làm acceptance baseline.

| Repo | Acceptance tại thời điểm snapshot | Kết quả |
|---|---|---:|
...
Baseline 423 tests và M03 26 tests trong v1.4 đã lỗi thời do thêm regression test cho face-score normalization. Baseline 424/54 ở trên, đến lượt nó, cũng đã lỗi thời ngay trong ngày 2026-07-16 — xem §0 để lấy con số current thật (454/65).
...
2. Khóa mapping đầu tiên:
   - baseline curated non-sport: `cafe_girl_classic`, `west_lake_sunset_classic`, `street_style_classic`, `business_travel_classic`;
   - sport family: `sport_active`;
   - sport schema subject: `outfit_e_sport`;
   - sport variants: `mint_green`, `nike_pink_running`.
...
| Thay đổi | v1.4 | v1.5 |
|---|---|---|
| AI Studio baseline | 423 tests | 424 tests (snapshot giữa kỳ) → **454** current §0 |
| VenHo OS baseline | 37 tests | 54 tests (snapshot giữa kỳ) → **65** sau closeout §0 |
