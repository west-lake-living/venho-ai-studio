...
Worker heartbeat · stale-job recovery · retry matrix · lateness alert · budget ledger 70/85/100% → block paid call trừ override có ghi nhận · backup tự động verify được.
...
**Exit:** một inquiry test truy được về đúng một publication · null ≠ unavailable ≠ zero.
...
-> auto-approval phạm vi hẹp CHỈ KHI có quyết định riêng của Harry
   (KHÔNG BAO GIỜ áp dụng cho lane đặc biệt T7)
```

**Exit:** scorecard ≥9.3/10 trên golden set có version · ≥1 Skill `_productize` chạy được cho hotel #2 không sửa core · runbook/rollback/budget/ownership documented.
