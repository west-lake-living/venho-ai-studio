...
def run_content_pipeline(brief: dict) -> dict:
    if brief.get("status") != "LOCKED":
        raise ValueError("Only LOCKED briefs can generate final packages")
    package_id = str(uuid.uuid4())
    candidates = M05ContentBridge().generate_candidates(brief)
    selected = candidates[0]
    validation = M03ValidatorBridge().validate_package(brief, selected)
