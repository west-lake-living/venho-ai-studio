...
- Existing dotenv credential loader recovered the approved Gemini path without
  exposing a secret. Credential and adapter readiness PASS for
  `gemini-flash-latest`.
- Preflight PASS: immutable B05 artifact hash `ce58f0ac…a873e40`, manifest,
  locked `denoise=0.35` / `CFG=6.0` / `steps=21` contract, and lineage all
  matched. `16 passed`, compileall PASS, diff-check PASS.
- Exactly one valid Gemini FACE_LOCAL call returned `88.90`, `revise`:
  eyes/brows `89`, facial shape `88`, mouth/chin `87`. This improves original
  B05 by `+0.40` and denoise `.40` by `+1.45`, but does not meet the PASS
  threshold. Provider calls `1`, retries `0`, GPU jobs `0`, artifacts `0`.
- Final state: Boundary `9/9 PASS`, FACE_LOCAL `8/9 PASS`, SCENARIO_GLOBAL
  `9/9 PASS`, pending `0`, quality `FAIL`, provider hold `RECOVERED`, feature
  `OFF`, promotion `NO`, architecture unchanged. Evidence:
...
- Rollback baseline was fixed at denoise `0.35`, CFG `6.0`, steps `20`.
  Exactly one final parameter decision was selected: `steps 20 -> 21`, the
  smallest higher value in the approved `[8,60]` range; denoise and CFG were
  unchanged.
- T2 contract/offline gate PASS: B05-only deterministic contract, A2 and
  workflow lineage authority, 9 focused tests, compileall, and diff check.
- T3 health gate found the approved HARRY-ROG Tailscale-ComfyUI worker
  offline. GPU jobs `0`, artifacts `0`, provider calls `0`; T4/T5 did not run.
  No retry, sweep, CFG experiment, provider switch, Nano call, or automatic
  follow-up phase was created.
- Protected quality remains Boundary `9/9 PASS`, FACE_LOCAL `8/9 PASS`,
  SCENARIO_GLOBAL `9/9 PASS`, pending `0`, quality `FAIL`, feature `OFF`,
  promotion `NO`, architecture unchanged. Evidence:
...
- `R1-P7-R1-TARGETED-AUTHORITATIVE-RECHECK = CLOSED / PASS` về execution,
  với `TARGETED_RECHECK_AUTHORIZED=TRUE`; đúng 5 case được chạy, không rerun
  Boundary hay 13 case baseline đã pass.
- FACE_LOCAL: `B05, B07` → `2/2 valid`, `1 pass / 1 fail`; B05 `88.50 /
  revise` còn fail, B07 `92.25 / approve`. SCENARIO_GLOBAL: `B05, B06, B09`
  → `3/3 valid`, `3 pass / 0 fail` với điểm `93.79`, `91.54`, `92.40`.
- Provider calls `5`, retries `0`; GPU/Nano/alternative-provider calls `0`.
  Tất cả raw/parsed evidence đều có lineage `VERIFIED`; evidence hash `32/32`.
- Final authoritative quality: Boundary `9/9 PASS`, FACE_LOCAL `8/9 PASS`,
  SCENARIO_GLOBAL `9/9 PASS`, pending `0`, `QUALITY_DISPOSITION = FAIL` do
  B05 FACE_LOCAL còn fail. Candidate v3 vẫn `OFF`, promotion `NO`,
  architecture unchanged.
- Next action: `TARGETED_REMEDIATION_R2_REQUIRES_SEPARATE_AUTHORIZATION`.
  Evidence:
...
- `R1-P5-R3-PROVIDER-RECOVERY-RECHECK = CLOSED / PROVIDER_BLOCKED`; explicit
  `PROVIDER_RECOVERY_RECHECK_AUTHORIZED=TRUE` was recorded. Start state was
  `R1-P5-R2 = CLOSED / PROVIDER_BLOCKED` with `PROVIDER_HOLD = ACTIVE`.
- Offline preflight passed before the live call: focused gate, authorization,
  one-call, Gemini adapter, schema/DTO, failure-classification, truncation,
  and lineage tests recorded `40 passed`; compileall PASS and
  `git diff --check` PASS.
- Exactly one live Gemini `gemini-flash-latest` probe ran on the existing
  authoritative `FACE_LOCAL/B01/sample-1` fixture with one transport attempt
  and no retry. The provider response did not return before the runner ended;
  classification is `PROVIDER_TIMEOUT`. No raw or parsed response exists.
- Recovery was not proven: `RECOVERY_PROBE = PROVIDER_BLOCKED`,
  `PROVIDER_RECOVERY_STATUS = PROVIDER_BLOCKED`, and `PROVIDER_HOLD = ACTIVE`.
  No FACE_LOCAL or SCENARIO_GLOBAL batch ran; `FACE_LOCAL = 0/9`,
  `SCENARIO_GLOBAL = 0/9`, `PENDING = 18`, `BOUNDARY = 9/9 PASS`, and quality
  remains `UNVALIDATED`.
- GPU jobs `0`, Nano calls `0`, alternative-provider calls `0`; no retry,
  provider/model switch, fallback, generation, architecture, feature-flag,
...
### Candidate v3 Quality Remediation R1 — R1-P5-R2 Provider Recovery Recheck (2026-09-01)

- `R1-P5-R2-PROVIDER-RECOVERY-RECHECK = CLOSED / PROVIDER_BLOCKED`; explicit
  `PROVIDER_RECOVERY_RECHECK_AUTHORIZED=TRUE` was recorded against the
  authoritative start state: `R1-P5 = CLOSED / PASS`, `R1-P5-R1 = CLOSED /
  PROVIDER_BLOCKED`, and `PROVIDER_HOLD = ACTIVE`.
- Offline preflight passed before live execution: the existing R1-P5 gate and
  provider recovery tests recorded `36 passed`, compileall PASS, and
  `git diff --check` PASS. The R2 harness regression suite also passed `38`
  focused tests in total.
- Exactly one live Gemini `gemini-flash-latest` probe ran on the existing
  authoritative `FACE_LOCAL/B01/sample-1` fixture. Transport attempts `1`,
  retries `0`, provider calls `1`; Gemini returned `503 UNAVAILABLE`. No
  complete response, raw response, or parsed response was produced.
- Recovery was not proven: `RECOVERY_PROBE = PROVIDER_BLOCKED`,
  `PROVIDER_RECOVERY_STATUS = PROVIDER_BLOCKED`, and `PROVIDER_HOLD = ACTIVE`.
  No FACE_LOCAL or SCENARIO_GLOBAL batch ran; `FACE_LOCAL = 0/9`,
  `SCENARIO_GLOBAL = 0/9`, `PENDING = 18`, `BOUNDARY = 9/9 PASS`, and quality
  remains `UNVALIDATED`.
- GPU jobs `0`, Nano calls `0`, alternative-provider calls `0`; no provider or
  model switch, fallback, retry, generation, architecture, feature-flag, or
  promotion change occurred. Candidate v3 remains `OFF`; promotion remains
  `NO`.
- Evidence:
...
- `R1-P5 = CLOSED / PASS` as a control-plane/state-machine hardening task;
  no live provider probe or quality evaluation was authorized or executed.
- The authoritative Gemini hold now has explicit states:
  `ACTIVE -> RECOVERY_CHECK_AUTHORIZED -> RECOVERY_PROBE_IN_PROGRESS ->
  RECOVERED`, with fail-closed transitions back to `ACTIVE` when recovery is
  not proven.
- Recovery requires strict human authorization
  `PROVIDER_RECOVERY_RECHECK_AUTHORIZED=TRUE`; absent, `FALSE`, malformed, or
  unknown values leave the hold active and permit `0` provider calls. The
  recovery probe budget is exactly `1`; R1-P5 itself starts no probe.
- ACTIVE hold blocks FACE_LOCAL, SCENARIO_GLOBAL, bulk execution, automatic
  provider/model fallback, and Nano/GPU paths. A recovered hold still does not
  authorize the 18 pending evaluations; those require a separate task and
  explicit resume authorization.
- Offline result: `28 passed`, compileall PASS, and `git diff --check` PASS.
  `PROVIDER_CALLS = 0`, `FACE_LOCAL = 0/9`, `SCENARIO_GLOBAL = 0/9`,
  `PENDING = 18`, `BOUNDARY = 9/9 PASS`, quality remains `UNVALIDATED`,
  feature flag remains `OFF`, and production promotion remains `NO`.
- Evidence:
