...
### ⚠️ Phát hiện đáng ngờ — Face Validator có thể chấm điểm theo khuôn mẫu, chưa xác minh (2026-07-17)
Sau khi sửa prompt sinh ảnh (expression/technical_quality) và chạy live lại E1, **5 category score ra giống hệt tuyệt đối** lần chạy trước đó (`facial_shape 90, eyes 85, hair 80, expression 75, technical_quality 70`) dù ảnh sinh ra khác và prompt đã đổi. Nghi ngờ: model chấm điểm theo pattern mặc định, không thực sự phân biệt input khác nhau (có thể do `temperature=0.0` + prompt cấu trúc cố định khiến model "đoán" cùng 1 bộ điểm hợp lý). **Chưa điều tra** — Harry quyết định debug sau, tránh tốn thêm phí. Trước khi tiếp tục tinh chỉnh prompt ảnh để "kéo điểm", nên xác minh giả thuyết này trước (ví dụ: chạy cùng 1 input 2-3 lần xem điểm có luôn giống hệt không).
...
- **JOB-01 — PARTIALLY ACCURATE, có gap thật chưa fix.** File-backed job store, status enum đủ 6 trạng thái, cancel thật sự kill subprocess (không phải flag ảo) — đều đúng. **Gap:** nếu server restart giữa lúc đang generate, job record kẹt vĩnh viễn ở `generating` — không có cơ chế reconcile/resume khi server khởi động lại (in-flight `AbortController` map là in-memory, mất khi restart). Cancel/DELETE endpoint cũng chưa có test. **Chưa fix — cần Harry quyết định có ưu tiên không** (đây là edge case chỉ xảy ra khi deploy lại server giữa lúc đang chạy job thật).
